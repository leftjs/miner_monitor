...
class GPU_INFO():
    def __init__(self, d):
        ...
        self.current_temp = int(d['temperature']['gpu_temp'].replace('C', '')) # C
        self.current_graphics_clock =  int(d['clocks']['graphics_clock'].replace('MHz', '')) # Mhz
        self.max_graphics_clock = int(d['max_clocks']['graphics_clock'].replace('MHz', '')) # Mhz
        self.current_mem_clock = int(d['clocks']['mem_clock'].replace('MHz', '')) # Mhz
        self.max_mem_clock = int(d['max_clocks']['mem_clock'].replace('MHz', '')) # Mhz
...
if __name__ == '__main__':
    ...
    for gpu in gpus:
        ...
        gpu_string = ','.join([str(value) for key, value in gpu.__dict__.items()])
